      ...
      <div className="relative h-full w-full -mt-2">
        <div className="absolute top-0 left-0 h-full w-full">
          {svgCheckpointItems.map((item, index) => (
            <div
              className={`w-full h-full absolute top-0 ${index === 0 ? "flex" : "hidden"
                }`}
              key={`${(item as CheckpointNode).title}-${index}`}
            >
              {svgCheckpointItems.map((subItem, subIndex) => (
                <Image
                  className={`w-full absolute top-0 object-contain slide-${subIndex + 1}`}
                  src={(subItem as CheckpointNode).slideImage || ""}
                  key={`${(subItem as CheckpointNode).title}-${subIndex}`}
                  alt="Timeline"
                  layout="fill"
                />
              ))}
